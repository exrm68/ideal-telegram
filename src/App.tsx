...
const App: React.FC = () => {
  ...
  const [maintenance, setMaintenance] = useState<null|{message:string;startTime:string;endTime:string;channelLink:string;appName:string}>(null);
    ...
    const unsubscribeSettings = onSnapshot(doc(db, 'settings', 'config'), (doc) => {
        if (doc.exists()) {
            ...
            if (data.maintenanceEnabled) {
              setMaintenance({ message: data.maintenanceMessage || 'আমাদের সার্ভার আপডেট চলছে।', startTime: data.maintenanceStartTime || '', endTime: data.maintenanceEndTime || '', channelLink: data.channelLink || '', appName: data.appName || 'Cineflix' });
            } else {
              setMaintenance(null);
            }
